...
public class ShortcutsDAO {
  ...
  public List<Shortcut> getAllShortcutsOfUser(final int building) {
    Query q = em.createNamedQuery("Shortcut.findAllInBuilding");
  ...
  public Shortcut saveShortcut(final Shortcut shortcut) {
    Shortcut instance = em.find(Shortcut.class, shortcut.getId(), LockModeType.PESSIMISTIC_WRITE);
    ...
    Device device = em.find(Device.class, deviceID, LockModeType.PESSIMISTIC_READ);
  ...
  public void deleteShortcut(final int id) {
    Shortcut instance = em.find(Shortcut.class, id, LockModeType.PESSIMISTIC_WRITE);
    em.remove(instance);
  }
}
